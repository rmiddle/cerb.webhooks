<form action="{devblocks_url}{/devblocks_url}" method="post" id="frmWebhookHandlerPeek">
...
{include file="devblocks:cerberusweb.core::internal/custom_fieldsets/peek_custom_fieldsets.tpl" context='cerberusweb.contexts.webhook_handler' context_id=$model->id}

{foreach from=$webhook_handler_engines item=engine key=engine_id}
...
<fieldset style="display:none;" class="delete">
	...
	<div>
		Are you sure you want to delete this webhook handler?
	</div>
...
<div class="buttons">
	<button type="button" class="submit" onclick="genericAjaxPopupPostCloseReloadView(null,'frmWebhookHandlerPeek','{$view_id}', false, 'webhook_handler_save');"><span class="cerb-sprite2 sprite-tick-circle"></span> {$translate->_('common.save_changes')|capitalize}</button>
...
<div style="float:right;">
	<a href="{devblocks_url}c=profiles&type=webhook_handler&id={$model->id}-{$model->name|devblocks_permalink}{/devblocks_url}">view full record</a>
</div>
<br clear="all">
{/if}
</form>
...
<script type="text/javascript">
	...
	$popup.one('popup_open', function(event,ui) {
		...
		$(this).dialog('option','title',"{'Webhook Handler'|escape:'javascript' nofilter}");
		
		$(this).find('input:text:first').focus();
		
		// Webhook handler engine fieldsets
